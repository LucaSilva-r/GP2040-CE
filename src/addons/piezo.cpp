...
bool PiezoInput::available() {
    return Storage::getInstance().getAddonOptions().piezoOptions.enabled;
}

void PiezoInput::setup() {
    const PiezoOptions& piezoOptions = Storage::getInstance().getAddonOptions().piezoOptions;
    
    // Setup our ADC Pair of Sticks
    adc_pairs[0].x_pin = piezoOptions.analogAdc1PinX;
    adc_pairs[0].y_pin = piezoOptions.analogAdc1PinY;
    adc_pairs[0].analog_invert = piezoOptions.analogAdc1Invert;
    adc_pairs[0].analog_dpad = piezoOptions.analogAdc1Mode;
    adc_pairs[0].ema_option = piezoOptions.analog_smoothing;
    adc_pairs[0].ema_smoothing = piezoOptions.smoothing_factor / 1000.0f;
    adc_pairs[0].error_rate = piezoOptions.analog_error / 1000.0f;
    adc_pairs[0].in_deadzone = piezoOptions.inner_deadzone / 100.0f;
    adc_pairs[0].out_deadzone = piezoOptions.outer_deadzone / 100.0f;
    adc_pairs[0].auto_calibration = piezoOptions.auto_calibrate;
    adc_pairs[0].forced_circularity = piezoOptions.forced_circularity;
    adc_pairs[1].x_pin = piezoOptions.analogAdc2PinX;
    adc_pairs[1].y_pin = piezoOptions.analogAdc2PinY;
    adc_pairs[1].analog_invert = piezoOptions.analogAdc2Invert;
    adc_pairs[1].analog_dpad = piezoOptions.analogAdc2Mode;
    adc_pairs[1].ema_option = piezoOptions.analog_smoothing2;
    adc_pairs[1].ema_smoothing = piezoOptions.smoothing_factor2 / 1000.0f;
    adc_pairs[1].error_rate = piezoOptions.analog_error2 / 1000.0f;
    adc_pairs[1].in_deadzone = piezoOptions.inner_deadzone2 / 100.0f;
    adc_pairs[1].out_deadzone = piezoOptions.outer_deadzone2 / 100.0f;
    adc_pairs[1].auto_calibration = piezoOptions.auto_calibrate2;
    adc_pairs[1].forced_circularity = piezoOptions.forced_circularity2;
...
float PiezoInput::readPin(int stick_num, Pin_t pin_adc, uint16_t center) {
...
float PiezoInput::emaCalculation(int stick_num, float ema_value, float ema_previous) {
    return (adc_pairs[stick_num].ema_smoothing * ema_value) + ((1.0f - adc_pairs[stick_num].ema_smoothing) * ema_previous);
}

uint16_t PiezoInput::map(uint16_t x, uint16_t in_min, uint16_t in_max, uint16_t out_min, uint16_t out_max) {
    return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}

float PiezoInput::magnitudeCalculation(int stick_num, piezo_instance & adc_inst) {
...
void PiezoInput::radialDeadzone(int stick_num, piezo_instance & adc_inst) {
